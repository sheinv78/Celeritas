...
public class RealModulationsAreHeardWhereAMusicianHearsThemTests
{
    ...
    public static TheoryData<string> OpeningsSequencesAndTremolos => [.. OpeningsSequencesAndTremolosPassages.Table.Select(p => p.Name)];

    public static TheoryData<string> RefutedKeysPausesAndDecoratedCloses => [.. RefutedKeysPausesAndDecoratedClosesPassages.Table.Select(p => p.Name)];
    ...
    public static TheoryData<string> OrdinaryMusic => [.. OrdinaryMusicPassages.Table.Select(p => p.Name)];

    public static TheoryData<string> TheLastReading => [.. TheLastReadingPassages.Table.Select(p => p.Name)];
    ...
    [Theory]
    [MemberData(nameof(OpeningsSequencesAndTremolos))]
    public void TheKeyTrajectoryHearsTheOpeningsSequencesAndTremolosAsAMusicianDoesInEveryKey(string name) =>
        AssertTheTrajectoryHears(OpeningsSequencesAndTremolosPassages.Table.Single(p => p.Name == name));

    [Theory]
    [MemberData(nameof(OpeningsSequencesAndTremolos))]
    public void TheModulationDetectorHearsTheOpeningsSequencesAndTremolosAsAMusicianDoesInEveryKey(string name) =>
        AssertTheDetectorHears(OpeningsSequencesAndTremolosPassages.Table.Single(p => p.Name == name));

    [Theory]
    [MemberData(nameof(RefutedKeysPausesAndDecoratedCloses))]
    public void TheKeyTrajectoryHearsTheRefutedKeysPausesAndDecoratedClosesAsAMusicianDoesInEveryKey(string name) =>
        AssertTheTrajectoryHears(RefutedKeysPausesAndDecoratedClosesPassages.Table.Single(p => p.Name == name));

    [Theory]
    [MemberData(nameof(RefutedKeysPausesAndDecoratedCloses))]
    public void TheModulationDetectorHearsTheRefutedKeysPausesAndDecoratedClosesAsAMusicianDoesInEveryKey(string name) =>
        AssertTheDetectorHears(RefutedKeysPausesAndDecoratedClosesPassages.Table.Single(p => p.Name == name));

    [Theory]
    [MemberData(nameof(MetresPedalsAndSharedRegisters))]
    public void TheKeyTrajectoryHearsTheMetresPedalsAndSharedRegistersAsAMusicianDoesInEveryKey(string name) =>
        AssertTheTrajectoryHears(MetresPedalsAndSharedRegistersPassages.Table.Single(p => p.Name == name));
    ...
    [Theory]
    [MemberData(nameof(OrdinaryMusic))]
    public void TheKeyTrajectoryHearsTheOrdinaryMusicAsAMusicianDoesInEveryKey(string name) =>
        AssertTheTrajectoryHears(OrdinaryMusicPassages.Table.Single(p => p.Name == name));

    [Theory]
    [MemberData(nameof(OrdinaryMusic))]
    public void TheModulationDetectorHearsTheOrdinaryMusicAsAMusicianDoesInEveryKey(string name) =>
        AssertTheDetectorHears(OrdinaryMusicPassages.Table.Single(p => p.Name == name));

    [Theory]
    [MemberData(nameof(TheLastReading))]
    public void TheKeyTrajectoryHearsTheLastReadingAsAMusicianDoesInEveryKey(string name) =>
        AssertTheTrajectoryHears(TheLastReadingPassages.Table.Single(p => p.Name == name));

    [Theory]
    [MemberData(nameof(TheLastReading))]
    public void TheModulationDetectorHearsTheLastReadingAsAMusicianDoesInEveryKey(string name) =>
        AssertTheDetectorHears(TheLastReadingPassages.Table.Single(p => p.Name == name));
    ...
    public void EveryLoopSilenceAndClosePassageIsBuiltAsItIsDescribed()
    {
        ...
        Assert.True(LoopsSilencesAndClosesPassages.Table.Count >= 28);
        Assert.Equal(names.Count, names.Distinct().Count());
        var earlier = RealModulationPassages.All.Concat(RealModulationPassages.HeldOut).Concat(RealModulationPassages.ReviewerHeldOut)
            .Concat(RealModulationPassages.ThirdReviewerHeldOut).Concat(RealModulationPassages.FourthReviewerHeldOut)
    ...
    [Fact]
    public void EveryOpeningSequenceAndTremoloPassageIsBuiltAsItIsDescribed()
    {
        foreach (var passage in OpeningsSequencesAndTremolosPassages.Table)
        {
            AssertBuilds(passage);
        }

        var names = OpeningsSequencesAndTremolosPassages.Table.Select(p => p.Name).ToList();
        Assert.True(OpeningsSequencesAndTremolosPassages.Table.Count >= 40);
        Assert.Equal(names.Count, names.Distinct().Count());
        var earlier = RealModulationPassages.All.Concat(RealModulationPassages.HeldOut).Concat(RealModulationPassages.ReviewerHeldOut)
            .Concat(RealModulationPassages.ThirdReviewerHeldOut).Concat(RealModulationPassages.FourthReviewerHeldOut)
            .Concat(TexturesAndHomecomingsPassages.Table).Concat(AccompanimentTexturesPassages.Table)
            .Concat(LoopsSilencesAndClosesPassages.Table).Select(p => p.Name);
        Assert.Empty(names.Intersect(earlier));
    }

    [Fact]
    public void EveryRefutedKeyPauseAndDecoratedClosePassageIsBuiltAsItIsDescribed()
    {
        foreach (var passage in RefutedKeysPausesAndDecoratedClosesPassages.Table)
        {
            AssertBuilds(passage);
        }

        var names = RefutedKeysPausesAndDecoratedClosesPassages.Table.Select(p => p.Name).ToList();
        Assert.True(RefutedKeysPausesAndDecoratedClosesPassages.Table.Count >= 31);
        Assert.Equal(names.Count, names.Distinct().Count());
        var earlier = RealModulationPassages.All.Concat(RealModulationPassages.HeldOut).Concat(RealModulationPassages.ReviewerHeldOut)
            .Concat(RealModulationPassages.ThirdReviewerHeldOut).Concat(RealModulationPassages.FourthReviewerHeldOut)
            .Concat(TexturesAndHomecomingsPassages.Table).Concat(AccompanimentTexturesPassages.Table)
            .Concat(LoopsSilencesAndClosesPassages.Table).Concat(OpeningsSequencesAndTremolosPassages.Table).Select(p => p.Name);
        Assert.Empty(names.Intersect(earlier));
    }

    [Fact]
    public void EveryMetrePedalAndSharedRegisterPassageIsBuiltAsItIsDescribed()
    {
        foreach (var passage in MetresPedalsAndSharedRegistersPassages.Table)
        {
            AssertBuilds(passage);
        }
    ...
    public void EveryBarFromTheMusicAndSharedRegisterPassageIsBuiltAsItIsDescribed()
    {
        ...
        Assert.True(BarsFromTheMusicAndSharedRegistersPassages.Table.Length >= 33);
        Assert.Equal(names.Count, names.Distinct().Count());
        var earlier = RealModulationPassages.All.Concat(RealModulationPassages.HeldOut).Concat(RealModulationPassages.ReviewerHeldOut)
            .Concat(RealModulationPassages.ThirdReviewerHeldOut).Concat(RealModulationPassages.FourthReviewerHeldOut)
            .Concat(TexturesAndHomecomingsPassages.Table).Concat(AccompanimentTexturesPassages.Table)
    ...
    [Fact]
    public void EveryOrdinaryMusicPassageIsBuiltAsItIsDescribed()
    {
        foreach (var passage in OrdinaryMusicPassages.Table)
        {
            AssertBuilds(passage);
        }

        var names = OrdinaryMusicPassages.Table.Select(p => p.Name).ToList();
        Assert.True(OrdinaryMusicPassages.Table.Length >= 30);
        Assert.Equal(names.Count, names.Distinct().Count());
        var earlier = RealModulationPassages.All.Concat(RealModulationPassages.HeldOut).Concat(RealModulationPassages.ReviewerHeldOut)
            .Concat(RealModulationPassages.ThirdReviewerHeldOut).Concat(RealModulationPassages.FourthReviewerHeldOut)
            .Concat(TexturesAndHomecomingsPassages.Table).Concat(AccompanimentTexturesPassages.Table)
            .Concat(LoopsSilencesAndClosesPassages.Table).Concat(OpeningsSequencesAndTremolosPassages.Table)
            .Concat(RefutedKeysPausesAndDecoratedClosesPassages.Table).Concat(MetresPedalsAndSharedRegistersPassages.Table)
            .Concat(BarsFromTheMusicAndSharedRegistersPassages.Table).Select(p => p.Name);
        Assert.Empty(names.Intersect(earlier));
    }

    [Fact]
    public void EveryLastReadingPassageIsBuiltAsItIsDescribed()
    {
        foreach (var passage in TheLastReadingPassages.Table)
        {
            AssertBuilds(passage);
        }

        var names = TheLastReadingPassages.Table.Select(p => p.Name).ToList();
        Assert.True(TheLastReadingPassages.Table.Length >= 26);
        Assert.Equal(names.Count, names.Distinct().Count());
        var earlier = RealModulationPassages.All.Concat(RealModulationPassages.HeldOut).Concat(RealModulationPassages.ReviewerHeldOut)
            .Concat(RealModulationPassages.ThirdReviewerHeldOut).Concat(RealModulationPassages.FourthReviewerHeldOut)
            .Concat(TexturesAndHomecomingsPassages.Table).Concat(AccompanimentTexturesPassages.Table)
            .Concat(LoopsSilencesAndClosesPassages.Table).Concat(OpeningsSequencesAndTremolosPassages.Table)
            .Concat(RefutedKeysPausesAndDecoratedClosesPassages.Table).Concat(MetresPedalsAndSharedRegistersPassages.Table)
            .Concat(BarsFromTheMusicAndSharedRegistersPassages.Table).Concat(OrdinaryMusicPassages.Table).Select(p => p.Name);
        Assert.Empty(names.Intersect(earlier));
    }
