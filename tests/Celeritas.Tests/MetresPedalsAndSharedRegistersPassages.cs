...
/// (one modulating passage written in 4/4, 3/4, 6/8 and 5/4 with the chord lengths to match, and a
/// piece that changes metre at the modulation), the PEDAL and what is still sounding (a pedal that
/// changes with the key, one inner voice held across the silent bar, the old key's chord tied
/// across the pivot, a fermata in the upper voices while the bass rests), the OPENING of a piece
...
/// Of the reviewer's thirty-four, twenty-two are here; twelve the library still reads otherwise
/// and are the next iteration's work, in three families. METRE, the limitation the tenth iteration
/// declared and measured rather than fixed: a 12/8 passage and chords of a bar and a half are
/// heard two bars late, a 3/4 piece whose phrases are three bars is not heard at all, and neither
/// is a slow piece of two-bar chords — the judge counts its phrases in whole notes because no road
/// passes it a metre. The PEDAL under silence: a silent chord-bar over a held <em>tonic</em> pedal
/// is still not a pause, where over a dominant pedal it now is. And the CLOSE: the closing chord's
/// figure above the tune, the tune crossing below the figure, and the last chord rolled as a
/// spread chord all lose the homecoming — the figure is told from the tune by register, and only
/// downwards.
/// </remarks>
