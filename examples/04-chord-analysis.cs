...
class ChordAnalysis
{
    static void Main()
    {
        ...
        var details = ChordAnalyzer.IdentifyWithDetails("C4 E4 G4 B4");
        Console.WriteLine($"\nChord details:");
        Console.WriteLine($"  Symbol: {details.Symbol}");
        Console.WriteLine($"  Root: {details.Root}");
        Console.WriteLine($"  Quality: {details.Quality}");
        Console.WriteLine($"  Inversion: {details.Inversion}");
        Console.WriteLine($"  Extensions: {string.Join(", ", details.Extensions)}");
    }
}
